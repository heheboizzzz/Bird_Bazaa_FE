...
import static com.technifysoft.bookapp.constrant.Constants.MAX_BYTES_PDF;
...
public class MyApplication extends Application {
    ...
    //created a static method to convert timestamp to proper date format to reuse again
    ...
    public static void loadPdfFromUrlSinglePage(String pdfUrl, String pdfTitle, PDFView pdfView, ProgressBar progressBar, TextView pagesTv) {
        ...
        ref.getBytes(MAX_BYTES_PDF).addOnSuccessListener(new OnSuccessListener<byte[]>() {
            ...
            public void onSuccess(byte[] bytes) {
                ...
                //set to pdfview
                pdfView.fromBytes(bytes).pages(0) //show only first page
                        .spacing(0).swipeHorizontal(false).enableSwipe(false).onError(new OnErrorListener() {
                            @Override
    ...
    public static void addToFavorite(Context context, String bookId) {
        ...
        if (firebaseAuth.getCurrentUser() == null) {
            ...
            ref.child(firebaseAuth.getUid()).child("Favorites").child(bookId).setValue(hashMap).addOnSuccessListener(new OnSuccessListener<Void>() {
                ...
                @Override
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(context, "Failed to add to cart due to " + e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    ...
    public static void removeFromFavorite(Context context, String bookId) {
        //add remove if user is logged in
        //1)Check if user is logged in
        FirebaseAuth firebaseAuth = FirebaseAuth.getInstance();
        if (firebaseAuth.getCurrentUser() == null) {
            ...
            ref.child(firebaseAuth.getUid()).child("Favorites").child(bookId).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
                public void onSuccess(Void unused) {
                    Toast.makeText(context, "Removed from your cart list...", Toast.LENGTH_SHORT).show();
                }
            }).addOnFailureListener(new OnFailureListener() {
                @Override
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(context, "Failed to remove from cart due to " + e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
