package com.technifysoft.bookapp.adapter;
...
public class AdapterComment extends RecyclerView.Adapter<AdapterComment.HolderComment> {
    ...
    public void onBindViewHolder(@NonNull HolderComment holder, int position) {
        ...
        //don't have user's name, profile picture, so load it using uid we stored in each comment
    ...
    private void deleteComment(ModelComment modelComment, HolderComment holder) {
        ...
        builder.setTitle("Delete Comment").setMessage("Are you sure you want to delete this comment?").setPositiveButton("DELETE", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
                //Delete from dialog clicked, begin delete

                DatabaseReference ref = FirebaseDatabase.getInstance().getReference("Books");
                ref.child(modelComment.getBookId()).child("Comments").child(modelComment.getId()).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
